...
def devis_formation(request):
    """Page de demande de devis pour les formations."""
    if request.method == "POST":
        company_name = request.POST.get("company_name", "").strip()
        rccm = request.POST.get("rccm", "").strip()
        secteur = request.POST.get("secteur", "").strip()
        taille_entreprise = request.POST.get("taille_entreprise", "").strip()
        adresse = request.POST.get("adresse", "").strip()
        civilite = request.POST.get("civilite", "mr").strip()
        ...
        type_formation = request.POST.get("type_formation", "").strip()
        domaines = request.POST.get("domaines", "").strip()
        nombre_participants = request.POST.get("nombre_participants", "10")
        budget = request.POST.get("budget", "500000")
        date_debut_souhaitee = request.POST.get("date_debut_souhaitee", "").strip()
        duree_souhaitee = request.POST.get("duree_souhaitee", "").strip()
        lieu_formation = request.POST.get("lieu_formation", "nos-locaux").strip()
        objectifs = request.POST.get("objectifs", "").strip()
        public_cible = request.POST.get("public_cible", "").strip()

        # Validation des champs obligatoires
        if not company_name or not secteur or not taille_entreprise or not fonction or not nom_complet or not email or not telephone or not objectifs:
            messages.error(request, "Veuillez remplir tous les champs obligatoires.")
            return redirect("core:devis_formation")

        from core.models import DevisFormation
        from datetime import datetime

        date_debut = None
        if date_debut_souhaitee:
            try:
                date_debut = datetime.strptime(date_debut_souhaitee, "%Y-%m-%d").date()
            except (ValueError, TypeError):
                date_debut = None

        devis = DevisFormation(
            ...
            domaines=domaines,
            nombre_participants=int(nombre_participants) if nombre_participants.isdigit() else 10,
            budget=budget,
        ...
        if request.FILES.get("fichier_joint"):
            devis.fichier_joint = request.FILES["fichier_joint"]

        devis.save()

        messages.success(
            request,
            f"Merci {nom_complet} ! Votre demande de devis a été envoyée avec succès. "
            f"Notre équipe vous contactera sous 24 heures avec un devis personnalisé."
        )
        return redirect("core:devis_formation")

    return render(request, "core/demandeformationdevis.html")
